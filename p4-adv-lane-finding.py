...
def main_pipeline(input):
    ...
    text_output_1 = "Radius of curvature: " + str(round(left_curverad, 2)) + " m"
    text_output_2 = "Distance from center: " + str(round(distance_center, 2)) + " m"
    cv2.putText(image, text_output_1, (65, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 0, 0))
    cv2.putText(image, text_output_2, (65, 130), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 0, 0))
...
def process_video(img):
    ...
    image_export = main_pipeline(image)
...
if video_input:
    ...
    clip_output.write_videofile("project_video_lane.mp4", audio=False)
else:
    file_list = os.listdir("test_images/")  # import all image of test folder
    print("Processing images from folder test_images.")
    for file_name in file_list:
        file_dir = 'test_images/' + file_name
        image = mpimg.imread(file_dir)
        image_export = main_pipeline(image)
        # export new image
        file_dir = 'output_images/' + file_name
        mpimg.imsave(file_dir, image_export)
    print("Images processed and exported.")
